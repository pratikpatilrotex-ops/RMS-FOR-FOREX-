...
if uploaded is not None:
    ...
    if result is not None:
        ...
        with tab_swap:
            ...
            ✔ Symbol suffix handled automatically  
            ✔ BUY / SELL swap separated  
            ✔ **Manual Contract Size & Pip Value Override**
            """)
            ...
            # ----------------------------------------------
            # INTELLIGENT DEFAULT CONTRACT & PIP CALCULATOR
            # ----------------------------------------------
            def get_contract_and_point_smart(symbol):
                """
                Smart defaults based on symbol type.
                Returns: (ContractSize, Point)
                """
                symbol = str(symbol).upper()
                ...
                # JPY pairs (3-digit pip)
                elif "JPY" in symbol:
                    return 100000, 0.001

                # Indices (common examples)
                elif any(x in symbol for x in ["US30", "NAS100", "SPX500", "GER40", "UK100"]):
                    return 1, 0.01

                # Oil/Energy
                elif any(x in symbol for x in ["WTI", "BRENT", "USOIL", "UKOIL"]):
                    return 1000, 0.01
                ...
                # Default: Standard Forex (5-digit)
                else:
                    return 100000, 0.00001
            ...
            # ----------------------------------------------
            # CREATE SYMBOL MASTER TABLE (Unique Symbols)
            # ----------------------------------------------
            unique_symbols = swap_df[["SymbolPrefix"]].drop_duplicates().copy()

            # Auto-populate with smart defaults
            unique_symbols[["ContractSize", "Point"]] = unique_symbols.apply(
                lambda row: pd.Series(get_contract_and_point_smart(row["SymbolPrefix"])),
                axis=1
            )

            # Initialize swap rates to 0
            if "swap_rates_master" not in st.session_state:
                st.session_state.swap_rates_master = unique_symbols.copy()

            # ----------------------------------------------
            # MANUAL OVERRIDE TABLE: Contract & Pip
            # ----------------------------------------------
            st.markdown("### 📝 Manual Symbol Configuration")
            st.markdown("**Edit Contract Size and Pip Value for each symbol:**")

            symbol_config = st.data_editor(
                st.session_state.swap_rates_master,
                use_container_width=True,
                num_rows="dynamic",
                key="symbol_config_editor",
                column_config={
                    "SymbolPrefix": st.column_config.TextColumn("Symbol", disabled=True),
                    "ContractSize": st.column_config.NumberColumn(
                        "Contract Size",
                        help="Standard lot size (e.g., 100000 for forex, 100 for gold)",
                        min_value=0.01,
                        step=1
                    ),
                    "Point": st.column_config.NumberColumn(
                        "Pip/Point Value",
                        help="Minimum price movement (0.00001 for 5-digit, 0.001 for 3-digit)",
                        min_value=0.000001,
                        step=0.00001,
                        format="%.5f"
                    )
            ...
            st.session_state.swap_rates_master = symbol_config

            # Merge with swap_df
            swap_df = swap_df.merge(
                symbol_config[["SymbolPrefix", "ContractSize", "Point"]],
                on="SymbolPrefix",
                how="left"
            )

            # ----------------------------------------------
            # MANUAL SWAP RATE OVERRIDE (BUY/SELL)
            # ----------------------------------------------
            st.markdown("### 💵 Manual Swap Rate Override (Per Symbol & Direction)")

            # Create unique symbol + order type combinations
            unique_combinations = swap_df[["SymbolPrefix", "OrderType"]].drop_duplicates().copy()
            unique_combinations["SwapRate"] = 0.0

            if "swap_rates_override" not in st.session_state:
                st.session_state.swap_rates_override = unique_combinations.copy()

            swap_rates_edited = st.data_editor(
                st.session_state.swap_rates_override,
                use_container_width=True,
                num_rows="dynamic",
                key="swap_rates_editor",
                column_config={
                    "SymbolPrefix": st.column_config.TextColumn("Symbol", disabled=True),
                    "OrderType": st.column_config.TextColumn("Type", disabled=True),
                    "SwapRate": st.column_config.NumberColumn(
                        "Swap Rate (points)",
                        help="Swap charge in points (positive = charge, negative = credit)",
                        step=0.01,
                        format="%.2f"
                    )
                }
            ...
            st.session_state.swap_rates_override = swap_rates_edited

            # Merge swap rates
            swap_df = swap_df.merge(
                swap_rates_edited[["SymbolPrefix", "OrderType", "SwapRate"]],
                on=["SymbolPrefix", "OrderType"],
                how="left"
            )

            # ----------------------------------------------
            # Swap Money Calculation (ACCURATE FORMULA)
            # ----------------------------------------------
            swap_df["SwapRate"] = pd.to_numeric(swap_df["SwapRate"], errors="coerce").fillna(0)
            swap_df["Volume"] = pd.to_numeric(swap_df["Volume"], errors="coerce").fillna(0)
            swap_df["SwapDays_Final"] = pd.to_numeric(swap_df["SwapDays_Final"], errors="coerce").fillna(0)
            swap_df["ContractSize"] = pd.to_numeric(swap_df["ContractSize"], errors="coerce").fillna(100000)
            swap_df["Point"] = pd.to_numeric(swap_df["Point"], errors="coerce").fillna(0.00001)

            # Formula: Swap Money = SwapRate × ContractSize × Point × Volume × SwapDays
            swap_df["SwapMoney"] = (
                    swap_df["SwapRate"]
                    * swap_df["ContractSize"]
                    * swap_df["Point"]
            ...
            st.dataframe(
                swap_df[[
                    "Symbol", "OrderType", "Volume",
                    "Open Time", "Close Time", "SwapDays_Final",
                    "ContractSize", "Point", "SwapRate", "SwapMoney"
                ]],
                use_container_width=True
            )

            # ----------------------------------------------
            # BUY vs SELL Summary + TOTAL ROW
            # ----------------------------------------------
            cf_trades = swap_df[swap_df["SwapDays_Final"] > 0]
            ...
            buy_sell_summary = pd.concat([buy_sell_summary, total_row], ignore_index=True)
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                swap_df.to_excel(writer, index=False, sheet_name="Swap Per Trade")
                buy_sell_summary.to_excel(writer, index=False, sheet_name="Swap Summary")
                symbol_config.to_excel(writer, index=False, sheet_name="Symbol Config")
                swap_rates_edited.to_excel(writer, index=False, sheet_name="Swap Rates")

            buf.seek(0)

            st.download_button(
                "📥 Download Complete Swap Report",
                data=buf,
                file_name="swap_complete_report.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
